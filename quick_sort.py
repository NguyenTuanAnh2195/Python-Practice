...
def get_pivot(arr, l, r):
    ...
    result = arr.index(get_median(arr, l, r))
    return result
...
def get_median(arr, l, r):
    ...
    last_val = temp_arr[-1]
    if len(temp_arr) % 2 == 0:
        middle_val = temp_arr[len(temp_arr) // 2]
    else:
        middle_val = temp_arr[(len(temp_arr) -1) // 2]
    selection_pool = [first_val, middle_val, last_val]
    exclusion_pool = [max(selection_pool), min(selection_pool)]
    if len(set(selection_pool)) <= 2:
        return max(selection_pool)
